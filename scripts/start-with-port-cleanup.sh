...
# Start Collector Platform with Port Cleanup
# This script frees up occupied ports and starts all services
...
free_port() {
  ...
  if is_port_in_use "$port"; then
    ...
    # In dev mode, we only care about Docker containers
    ...
    for pid in $pids; do
      ...
      # Everything will run in Docker containers anyway
      if [[ "$port" == "5432" ]] && [[ "$process_path" == *"postgres"* ]] && [[ "$process_path" != *"docker"* ]]; then
        ...
        if kill -TERM $pid 2>/dev/null; then
          ...
          print_warning "  Could not send TERM signal, trying KILL..."
          kill -KILL $pid 2>/dev/null
          sleep 2
        fi
        
        # Skip to next iteration to avoid duplicate kill attempts
        continue
      fi
...
main() {
  echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
  echo -e "${BLUE}   Collector Platform - Startup with Port Cleanup${NC}"
  echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
  echo ""
...
cleanup() {
  ...
  if [ -f /tmp/collector-nextjs.pid ]; then
    local pid=$(cat /tmp/collector-nextjs.pid)
    if kill -0 $pid 2>/dev/null; then
      kill $pid 2>/dev/null
      print_info "Stopped Next.js (PID: $pid)"
    fi
    rm -f /tmp/collector-nextjs.pid
  fi
}
