...
async function proxyRequest(
  ...
  try {
    ...
    const url = new URL(`${OFFERS_SERVICE_URL}/api/offers/${offerId}/line-items/${lineItemId}`);
    ...
    console.error('Offers line-item API proxy error:', error);
    return NextResponse.json(
      { 
        success: false, 
...
export async function PUT(
  ...
  return proxyRequest(request, id, lineItemId, 'PUT', body);
}
...
export async function DELETE(
  ...
  const { id, lineItemId } = await params;
  return proxyRequest(request, id, lineItemId, 'DELETE');
}
