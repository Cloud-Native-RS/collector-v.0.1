...
async function proxyRequest(
  ...
  try {
    ...
    const url = new URL(`${REGISTRY_SERVICE_URL}${path}`);
...
export async function GET(
  ...
  return proxyRequest(request, `/api/customers/${id}`);
}
...
export async function PUT(
  ...
  return proxyRequest(request, `/api/customers/${id}`, 'PUT', body);
}
...
export async function DELETE(
  ...
  const { id } = await params;
  return proxyRequest(request, `/api/customers/${id}`, 'DELETE');
}
