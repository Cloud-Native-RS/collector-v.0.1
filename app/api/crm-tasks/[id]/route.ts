...
async function proxyRequest(
  ...
  try {
    ...
    console.error('CRM Task API proxy error:', error);
    return NextResponse.json(
      { 
        success: false, 
...
export async function GET(
  ...
  return proxyRequest(request, `/api/tasks/${id}`);
}
...
export async function PUT(
  ...
  return proxyRequest(request, `/api/tasks/${id}`, 'PUT', body);
}
...
export async function DELETE(
  ...
  const { id } = await params;
  return proxyRequest(request, `/api/tasks/${id}`, 'DELETE');
}
