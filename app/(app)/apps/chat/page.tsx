...
import { ChatSidebar, ChatContent } from "@/app/(app)/apps/chat/components";
...
async function getChats() {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/(app)/apps/chat/data/chats.json")
  );
...
async function getChatUser(id: number) {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/(app)/apps/chat/data/contacts.json")
  );
